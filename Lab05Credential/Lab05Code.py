...
def credential_EncryptUserSecret(params, pub, priv):
    ...
    Wpriv = g.pt_mul(wpriv)
    Wa = g.pt_mul(wa)
    Wb = pub.pt_mul(wa) + g.pt_mul(wb)
...
def credential_Issuing(params, pub, ciphertext, issuer_params):
    ...
    # TODO 3
    r_prime = o.random()
    # new_a = g.pt_mul(r_prime) + (x1b * a)
    # new_b = pub.pt_mul(r_prime) + (x1b * b) + (x0 * u)
...
def credential_show(params, issuer_pub_params, u, u_prime, v):
    ...
    WCv = wv * u + wz1 * h
    WV = wr * (-g) + wz1 * X1
    c = to_challenge([g, h, u, X1, WCv, WV])
...
def credential_show_verify(params, issuer_params, tag, proof):
    ...
    c_prime = to_challenge([g, h, u, X1, WCv, WV])
...
def credential_show_pseudonym(params, issuer_pub_params, u, u_prime, v, service_name):
    ...
    c = to_challenge([g, h, u, X1, WCv, WV, WPseudonym])
...
def credential_show_verify_pseudonym(params, issuer_params, pseudonym, tag, proof, service_name):
    ...
    WCv = rv * u + rz1 * h + c * Cv
    WV = rr * (-g) + rz1 * X1 + c * V
    WPseudo = rv * N + c * pseudonym

    c_prime = to_challenge([g, h, u, X1, WCv, WV, WPseudo])
    return c == c_prime
